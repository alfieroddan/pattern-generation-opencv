...
try:
    ...
    from urllib import urlopen  # noqa F401
...
class NewOpenCVTests(unittest.TestCase):
    ...
    def find_file(self, filename, searchPaths=[], required=True):
        searchPaths = (
            searchPaths if searchPaths else [self.repoPath,
                                             self.extraTestDataPath]
        )
    ...
    def get_sample(self, filename, iscolor=None):
        ...
        if filename not in self.image_cache:
            ...
            with open(filepath, "rb") as f:
                filedata = f.read()
            self.image_cache[filename] = cv.imdecode(
                np.fromstring(filedata, dtype=np.uint8), iscolor
            )
    ...
    def bootstrap():
        ...
        parser.add_argument(
            "--repo",
            help="use sample image files from local git repository \
                (path to folder), "
            "if not set, samples will be downloaded from github.com",
        )
        parser.add_argument(
            "--data",
            help="<not used> use data files from local folder \
            (path to folder)\
        ...
        try:
            NewOpenCVTests.extraTestDataPath = os.environ["OPENCV_TEST_DATA_PATH"]  # noqa E501
        except KeyError:
...
def isPointInRect(p, rect):
    if rect[0] <= p[0] and rect[1] <= p[1] and p[0] <= rect[2] and p[1] <= rect[3]:  # noqa E501
        return True
    else:
        return False
