...
import numpy as np
import cv2 as cv
from opencv_pattern.tests_common import NewOpenCVTests
import gen_pattern
    ...
    def test_aruco_dicts(self):
        try:
            ...
            for aruco_type_i in range(len(aruco_type)):
                ...
                aruco_dict = cv.aruco.getPredefinedDictionary(aruco_type[aruco_type_i])  # noqa E501
                board = cv.aruco.CharucoBoard(
                    (cols, rows), square_size, marker_size, aruco_dict
                )
                charuco_detector = cv.aruco.CharucoDetector(board)
                from_cv_img = board.generateImage(
                    (cols * square_size, rows * square_size)
                )

                # draw desk using svg
                fd1, filesvg = tempfile.mkstemp(prefix="out", suffix=".svg")
                os.close(fd1)
                fd2, filepng = tempfile.mkstemp(prefix="svg_marker", suffix=".png") # noqa E501
                os.close(fd2)
                ...
                try:
                    ...
                    pm = gen_pattern.PatternMaker(
                        ...
                        marker_size,
                        os.path.join(
                            basedir, aruco_type_str[aruco_type_i] + ".json.gz"
                        ),
                    )
                    ...
                    (
                        ...
                        marker_corners_svg,
                        marker_ids_svg,
                    ) = charuco_detector.detectBoard(from_svg_img)
                    (
                        _charucoCorners,
                        _charuco_ids_cv,
                        marker_corners_cv,
                        marker_ids_cv,
                    ) = charuco_detector.detectBoard(from_cv_img)
                    marker_corners_svg_map, marker_corners_cv_map = {}, {}
                    for i in range(len(marker_ids_svg)):
                        marker_corners_svg_map[int(marker_ids_svg[i][0])] = (
                            marker_corners_svg[i]
                        )
                    for i in range(len(marker_ids_cv)):
                        marker_corners_cv_map[int(marker_ids_cv[i][0])] = (
                            marker_corners_cv[i]
                        )
                    ...
                    for key_svg in marker_corners_svg_map.keys():
                        ...
                        np.testing.assert_allclose(marker_svg, marker_cv, 0.1, 0.1)  # noqa E501
                finally:
            ...
            for marker_s_rate in marker_sizes_rate:
                ...
                try:
                    ...
                    pm = gen_pattern.PatternMaker(
                        ...
                        "charuco_checkboard",
                        marker_size,
                        os.path.join(basedir, aruco_type_str + ".json.gz"),
                    )
                    pm.make_charuco_board()
                    pm.save()
                    drawing = svg2rlg(filesvg)
