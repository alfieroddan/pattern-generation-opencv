...
import json
import gzip
from opencv_pattern.svgfig import SVG, canvas
...
def main():
    ...
    parser.add_argument(
        "--from_config", help="generate pattern from config",
        dest="from_config", type=str
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    if args.from_config:
        yaml_dict = load_yaml(args.from_config)
    ...
    if "page_width" and "page_height" in args:
        ...
        # page size dict (ISO standard, mm) for easy lookup.
    ...
    # save config
    dict_args = vars(args)
    dict_args.pop("show_help")
    if dict_args["p_type"] != "charuco_board":
        dict_args.pop("dict_file")
        dict_args.pop("aruco_marker_size")

    # save to same dir as output
    if not args.from_config:
        yaml_file_path = args.output.split(".")[0]+".yaml"
        with open(yaml_file_path, "w+") as ff:
            yaml.dump(dict_args, ff)
